...
[![Version](https://img.shields.io/badge/version-v1.0.1-333333)](https://github.com/ENIGMA-K/PaperMind/releases)
...
- **元数据不用手填。** PDF 拖进去，LLM 自动从首页提取 DOI，通过 Crossref → OpenAlex 链查 ISSN、出版商、h-index，再补 CiteScore，按公式算出期刊影响力评分。精确到每一篇论文的期刊排名，全自动完成。
...
/ingest raw/artical/某论文.pdf   # 指定单篇录入
/ingest raw/artical/             # 批量录入整个目录
...
# 精读已录入论文：
/look-into Panza 2019                            # 全文通读 → 深度拆解 → 交叉验证
/look-into                                       # 列出所有待读论文让你选
...
**📥 `/ingest [路径]`** — 批量录入：自动查重 → DOI 提取 → 期刊评分 → 方法学评估 → 术语提取 → 写摘要  
**🔬 `/look-into [标题]`** — 单篇精读：全文通读 → 方法审查 → 数据提取 → 交叉验证 → 批判分析  
...
说「**已速览 / 已读 / 已分析**」→ 自动同步五级阅读进度（`unread → ingested → skimmed → read → analyzed`），全库论文的阅读深度一目了然
